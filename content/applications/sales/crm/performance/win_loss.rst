...
The :icon:`fa-pencil` :guilabel:`Filters` section allows users to add pre-made and custom filters to
...
Here's an example of a win/loss report for the current year with results grouped by
:guilabel:`Stage`, :guilabel:`Won/Lost` status, and :guilabel:`Salesperson`, allowing the viewer to
tell at a glance when leads and opportunities are lost or won and which salespeople were handling
them at the time.
...
:icon:`fa-pencil` :guilabel:`Filters` heading, ensure both :guilabel:`Opportunities` and
:guilabel:`Leads` have been enabled to capture results for both Opportunities and Leads. Click
:guilabel:`Custom Filter...` to bring up the *Custom filter* pop-up. Click on the first field in the
:guilabel:`Match any of the following rules:` section and select :guilabel:`Won/Lost`. Doing so
automatically sets the second field to :guilabel:`is equal to`. Set the third field to
:guilabel:`Won` and add another rule by clicking :guilabel:`New Rule`. Configure this new rule the
same as the first one, except be sure to set the third field to :guilabel:`Lost`. Finally, toggle
the :guilabel:`Include archived` switch in the top-right of the form and click :guilabel:`Search`.

.. important::
   Toggling the :guilabel:`Include archived` switch when creating win/loss reports guarantees that
...
activate the :guilabel:`Stage` option. Then click :guilabel:`Custom Group` and choose
:guilabel:`Won/Lost` from the drop-down menu. Finally, activate the :guilabel:`Salesperson` option.
By default, the data will appear with different results stacked on top of each other according to
their stage. To view the bars in this chart side-by-side, deactivate the :icon:`fa-database`
:guilabel:`(Stacked)` toggle.
